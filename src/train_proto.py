...
from meta_model import RobertaPOS, BertPOS
...
def train_func(sub_trains_, epoch, tokenizer, id2labels, label_sentence_dicts, soft_kmeans, unsup_data_ = None, test_example = None):
    ...
    #                   collate_fn=geposate_batch)  
    group_num = -(-EPISODE_NUM // 200)
    for g_num in range(group_num):
        data, dataset_chosen = geposate_episode(sub_trains_, epoch * group_num + g_num, min(200, EPISODE_NUM - g_num*200), max_seq_len, SUP_CLS_NUM, SUP_PER_CLS, QUR_PER_CLS, id2labels, label_sentence_dicts, 
        ...
        for j, (text, attention_mask, cls, orig_len, support_class, inv_support_classes) in enumerate(data):
            ...
            if unsup_data_ is not None:
                ...
                data = geposate_unsup_episode(unsup_data_[int(len_unsup_data / (N_EPOCHS * EPISODE_NUM) * cur_idx):int(len_unsup_data / (N_EPOCHS * EPISODE_NUM) * (cur_idx + 1))], 1, 64, max_seq_len, TEST_SUP_CLS_NUM, 0, id2label)
                ...
                for j1, (text, attention_mask, cls, orig_len, support_class, inv_support_classes) in enumerate(data): 
                    ...
                    example_data = geposate_testing_example(test_example, 64, max_seq_len, TEST_SUP_CLS_NUM, id2label)
...
def test(data_, epoch, id2labels, label_sentence_dicts, soft_kmeans, test_all_data = False, finetune = False, test_example = None):
    ...
    # data = DataLoader(data_, batch_size=256, collate_fn=geposate_batch)
    # data = geposate_episode(data_, epoch,  EPISODE_NUM, TEST_SUP_CLS_NUM, test_id2label, label_sentence_dict)
    if test_all_data:
        id2label = id2labels[0]
        label_sentence_dict = label_sentence_dicts[0]
        data_ = data_[0]
        if test_example is not None:
            data = geposate_testing_episode(data_, epoch+1, 64, max_seq_len, TEST_SUP_CLS_NUM, 0, id2label, label_sentence_dict)
        else:
            data = geposate_testing_episode(data_, epoch+1, 64, max_seq_len, TEST_SUP_CLS_NUM, SUP_PER_CLS, id2label, label_sentence_dict)
    else:
        data, dataset_chosen = geposate_episode(data_, epoch, 50, max_seq_len, TEST_SUP_CLS_NUM, SUP_PER_CLS, QUR_PER_CLS, id2labels, label_sentence_dicts, 
        gpu_num = gpu_num, uniform = False, all_o_sent = int(QUR_PER_CLS * o_sent_ratio))
...
def get_prob(data_, id2labels, label_sentence_dicts, test_example = None):
    ...
    data = geposate_testing_episode(data_, 1, 64, max_seq_len, TEST_SUP_CLS_NUM, 0, id2label, label_sentence_dicts[0])
    prob = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_pos', default='train.pos')
    parser.add_argument('--test_text', default='test.words')
    parser.add_argument('--test_pos', default='test.pos')
    parser.add_argument('--test_example', default='train.words')
    parser.add_argument('--test_example_pos', default='train.pos')
    parser.add_argument('--unsup_text', default='train_10percent.words')
    parser.add_argument('--unsup_pos', default='train_10percent.pos')
    ...
    train_poss = [os.path.join(args.datapath, dataset, args.train_pos) for dataset in datasets]
    test_texts = [os.path.join(args.datapath, dataset, args.test_text) for dataset in datasets]
    test_poss = [os.path.join(args.datapath, dataset, args.test_pos) for dataset in datasets]
    test_examples = [os.path.join(args.datapath, dataset, args.test_example) for dataset in datasets]
    test_example_poss = [os.path.join(args.datapath, dataset, args.test_example_pos) for dataset in datasets]
    ...
    if not args.load_dataset:
        ...
        for train_text, train_pos, test_text, test_pos in zip(train_texts, train_poss, test_texts, test_poss):
            with open(train_pos) as fpos, open(train_text) as f:
                train_pos_tags, train_words = fpos.readlines(), f.readlines() 
            with open(test_pos) as fpos, open(test_text) as f:
                test_pos_tags, test_words = fpos.readlines(), f.readlines()      
            label2id, id2label = get_label_dict([train_pos_tags, test_pos_tags])
            train_pos_tags, train_words, train_label_sentence_dict = process_data(train_pos_tags, train_words, tokenizer, label2id, max_seq_len,base_model=base_model)
            test_pos_tags, test_words, test_label_sentence_dict = process_data(test_pos_tags, test_words, tokenizer, label2id, max_seq_len,base_model=base_model)

            sub_train_ = [[train_words[i], train_pos_tags[i]] for i in range(len(train_pos_tags))]
            sub_valid_ = [[test_words[i], test_pos_tags[i]] for i in range(len(test_pos_tags))]
    ...
    # label2ids, id2labels = [], []
    # train_words_list, train_pos_tags_list= [], []
    # processed_training_set, train_label_sentence_dicts = [], []
    # if not JUST_EVAL: 
    #     for train_text, train_pos in zip(train_texts, train_poss):
    #         with open(train_pos) as fpos, open(train_text) as f:
    #             train_pos_tags, train_words = fpos.readlines(), f.readlines()       
    #         label2id, id2label = get_label_dict([train_pos_tags])
    #         train_pos_tags, train_words, train_label_sentence_dict = process_data(train_pos_tags, train_words, tokenizer, label2id, max_seq_len)
    #         sub_train_ = [[train_words[i], train_pos_tags[i]] for i in range(len(train_pos_tags))]
    #         train_pos_tags_list.append(train_pos_tags)
    ...
    # with open(test_pos) as fpos, open(test_text) as f:
    #     test_pos_tags, test_words = fpos.readlines(), f.readlines()
    # test_label2id, test_id2label = get_label_dict([test_pos_tags])
    # test_pos_tags, test_words, test_label_sentence_dict = process_data(test_pos_tags, test_words, tokenizer, test_label2id, max_seq_len)
    # sub_valid_ = [[test_words[i], test_pos_tags[i]] for i in range(len(test_pos_tags))]
    
    if USE_EXAMPLE:
        with open(os.path.join(args.datapath, args.dataset, args.test_example_pos)) as fpos, open(os.path.join(args.datapath, args.dataset, args.test_example)) as f:
            test_example_pos_tags, test_example_words = fpos.readlines(), f.readlines()
        # test_label2id, test_id2label = get_label_dict([test_pos_tags])
        test_example_pos_tags, test_example_words, _ = process_data(test_example_pos_tags, test_example_words, tokenizer, label2ids[0], max_seq_len,base_model=base_model)
        sub_example_ = [[test_example_words[i], test_example_pos_tags[i]] for i in range(len(test_example_pos_tags))]
    ...
    if LOAD_MODEL:
        if 'checkpoint' not in args.load_model_name:
            ...
            if base_model == 'roberta':
                model = RobertaPOS.from_pretrained('roberta-base', output_attentions=False, output_hidden_states=False, \
                use_global = False, dataset_label_nums=dataset_label_nums, support_per_class = SUP_PER_CLS, cuda_device = 0, use_bias = False)
            elif base_model == 'bert':
                model = BertPOS.from_pretrained('bert-base-cased', output_attentions=False, output_hidden_states=False, \
                use_global = False, dataset_label_nums=dataset_label_nums, support_per_class = SUP_PER_CLS, cuda_device = 0, use_bias = False)
    ...
    with open(os.path.join(args.datapath, args.dataset, args.unsup_pos)) as fpos, open(os.path.join(args.datapath, args.dataset, args.unsup_text)) as f:
        unsup_pos_tags, unsup_words = fpos.readlines(), f.readlines()
    # test_label2id, test_id2label = get_label_dict([test_pos_tags])
    unsup_pos_tags, unsup_words, _ = process_data(unsup_pos_tags, unsup_words, tokenizer, label2ids[0], max_seq_len,base_model=base_model)
    # use prediction instead of ground truth
    unsup_data_ = [[unsup_words[i], unsup_pos_tags[i]] for i in range(len(unsup_words))]
