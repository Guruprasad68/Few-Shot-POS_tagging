...
from finetune_model import RobertaPOS, BertPOS
from eval_util import batch_span_eval
from data import *
from torch.utils.tensorboard import SummaryWriter

def geposate_batch(batch):
...
def geposate_batch_unsup(batch):
    text = [F.pad(torch.tensor(x[0]), (0,max_seq_len-len(x[0])), "constant", 1) for x in batch] # batch_size * max_seq_len 
    text = pad_sequence(text, batch_first = True)
    attention_mask = [torch.cat((torch.ones_like(torch.tensor(x[0])), torch.zeros(max_seq_len-len(x[0]), dtype=torch.int64)), dim=0)
        if len(x[0]) < max_seq_len else torch.ones_like(torch.tensor(x[0]))[:max_seq_len] for x in batch]
...
def train_func(processed_training_set, epoch, tokenizer, label_sentence_dicts, soft_kmeans, count_num = 0, unsup_data_iter = None):
    ...
    for i,d in enumerate(processed_training_set):
        one_dataset = DataLoader(d, batch_size=BATCH_SIZE, collate_fn=geposate_batch)
...
def test(data_, epoch, label_sentence_dicts, soft_kmeans, test_all_data = True, finetune = False):
    ...
    if test_all_data:
        ...
        for i,d in enumerate(data_):
            one_dataset = DataLoader(d, batch_size=64, collate_fn=geposate_batch)
            data.extend(one_dataset)
            dataset_chosen.extend([i for x in range(len(one_dataset))])
        # data = geposate_testing_episode(data_, epoch+1, TEST_SUP_CLS_NUM, test_id2label, label_sentence_dict)
    else:
        data, _ = geposate_episode(data_, epoch,  EPISODE_NUM, TEST_SUP_CLS_NUM, test_id2label, label_sentence_dict, use_multipledata = False)
    idx = 0
    f1ss = []
    pss = []
    rss = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unsup_pos', default=None)
    ...
    for round in range(args.few_shot_sets):
        ...
        if not args.load_dataset:
            for train_text, train_pos, test_text, test_pos in zip(train_texts, train_poss, test_texts, test_poss):
                ...
                train_pos_tags, train_words, train_label_sentence_dict = process_data(train_pos_tags, train_words, tokenizer, label2id, max_seq_len,base_model=base_model,use_truecase=args.use_truecase)
                test_pos_tags, test_words, test_label_sentence_dict = process_data(test_pos_tags, test_words, tokenizer, label2id, max_seq_len,base_model=base_model,use_truecase=args.use_truecase)
        ...
        if LOAD_MODEL:
            if 'checkpoint' not in args.load_model_name:
                ...
                if base_model == 'roberta':
                    model = RobertaPOS.from_pretrained('roberta-base', dataset_label_nums = dataset_label_nums, output_attentions=False, output_hidden_states=False, multi_gpus=False)
                elif base_model == 'bert':
                    model = BertPOS.from_pretrained('bert-base-cased', dataset_label_nums = dataset_label_nums, output_attentions=False, output_hidden_states=False, multi_gpus=False)
        ...
        if args.unsup_text is not None:
            ...
            if unsup_data_ is not None:
                ...
                data_loader = DataLoader(unsup_data_, batch_size=1*BATCH_SIZE, collate_fn=geposate_batch_unsup)
