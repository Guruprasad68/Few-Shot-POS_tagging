...
def geposate_episode(data_list, epoch, EPISODE_NUM, max_seq_len, sup_cls_num, SUP_PER_CLS, QUR_PER_CLS, id2labels, label_sentence_dicts, 
    ...
    logging.info(f"Geposating {episode_num} episodes, starting from {epoch*episode_num}!")
...
def geposate_testing_episode(data, episode_num, batch_size, max_seq_len, sup_cls_num, SUP_PER_CLS, id2label, label_sentence_dict, train_other_queries = False, train_other_labels = True):
    processed_data = []
...
def geposate_unsup_episode(data, episode_num, batch_size, max_seq_len, sup_cls_num, SUP_PER_CLS, id2label):
    processed_data = []
    np.random.seed(episode_num)
    support_class = {}
    inv_support_classes = {}
...
def geposate_testing_example(data, batch_size, max_seq_len, sup_cls_num, id2label, train_other_queries = False, train_other_labels = True):
    processed_data = []
    support_class = {}
    inv_support_classes = {}
    for cls in range(len(id2label)):
...
def process_data(pos_tags, sents, tokenizer, label2id, max_sequence_len, base_model='roberta', lower=False, use_truecase=False):
