...
def test(data_, epoch, id2labels, label_sentence_dicts, soft_kmeans, test_all_data = False, finetune = False, test_example = None):
    ...
    if test_all_data:
        ...
        if test_example is not None:
            data = geposate_testing_episode(data_, epoch+1, 32, max_seq_len, TEST_SUP_CLS_NUM, 0, id2label, label_sentence_dict)
        else:
            data = geposate_testing_episode(data_, epoch+1, 32, max_seq_len, TEST_SUP_CLS_NUM, SUP_PER_CLS, id2label, label_sentence_dict)
    else:
                    ...
                    if j == 0:
                        if use_multi_prototype:
                            ...
                            for k in range(support_group_num):
                                ...
                                example_data = geposate_testing_example(support_group, 32, max_seq_len, TEST_SUP_CLS_NUM, id2label, label_sentence_dict)
                                prototypes = None
                            ...
                            example_data = geposate_testing_example(test_example, 32, max_seq_len, TEST_SUP_CLS_NUM, id2label, label_sentence_dict)
...
if __name__ == "__main__":
    ...
    for round in range(args.few_shot_sets):
        ...
        if args.use_example == True: #False: ## Julia changed 4/23
