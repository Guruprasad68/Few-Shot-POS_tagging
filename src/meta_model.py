...
class RobertaPOS(RobertaForTokenClassification):
    ...
    def compute_prototypes(self, input_ids, 
        ...
        # print(embeds_per_class, embed_class_len)
        labels_numpy = labels.data#.cpu().numpy()
        # print(labels.data)

        # print(len(embeds_per_class))
        ...
        # print("size of first dimension", len(embeds_per_class))
        # if len(embeds_per_class) > 0:
        #   print("size of second dimension", len(embeds_per_class[0]))
        # print(np.array(embeds_per_class).shape)
        # print(np.array(embeds_per_class[0]).shape[0])
        # if np.array(embeds_per_class[0]).shape[0] == 0:
        #   # print(embeds_per_class)
        #   # print(labels)
        #   print(labels.data)
        #   print(labels.data.shape)
        # print(torch.tensor(embeds_per_class[0][0]).shape)

        # Here we compute embeddings
        # prototypes = [torch.zeros_like(embeds_per_class[0][0]) for _ in range(len(embeds_per_class))]

        # cuda0 = torch.device(self.cuda_device)

        prototypes = []
        for _ in range(len(embeds_per_class)):
          if np.array(embeds_per_class[0]).shape[0] > 0:
            # prototypes.append(torch.zeros_like(embeds_per_class[0][0]).to(cuda0))
            prototypes.append(torch.zeros_like(embeds_per_class[0][0]).cuda(self.cuda_device))
          else:
            # print('Appending 768 zeros')
            # prototypes.append(torch.zeros_like(torch.zeros(size=[768])).to(cuda0))
            prototypes.append(torch.zeros_like(torch.zeros(size=[768])).cuda(self.cuda_device))
...
class BertPOS(BertForTokenClassification):
